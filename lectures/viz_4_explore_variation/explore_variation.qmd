...
=======
Most of the movies have at least 10% positive ratings and a roughly uniform set of ratings. 
>>>>>>> Stashed changes
...
In the IMDB chart there are more values and so if you were to pick any 2 values you might not know that they are different. But with the other graph you have a higher chance that the two values are different since you have about 100 something in every single bin. 
...
:::
=======
- Seems like if there are more bins then there are more fine grained differences between the values across the bins. 
- Plot 1: With only 344 penguins, many 20 gram cells will be empty 
- Plot 2: No more 0 bins, concentrated around 3400 g. Lot easier to see than the first graph. 
- Plot 3: Not many larger than 6k, everyone is greater than 2k g
...
Trying to figure out the underlying distribution which gives this plot. 
...
Most diamons have low weight and a few are high weight. 
...
Trying to remove the white space because not very helpful to have the other values that are completely missing. 
...
:::
=======
- Focused around certain values so might be concerned about measurement error 
- Might be seeing clustering around whole numbers because from a marketing perspective, it is better around 1 or 2 etc 
- Clumping around certain values of 0.33, 0.50, 0.67 
- External considerations may give rise to this distribution 

Point of this is where increased binning might give rise to new patterns. 
...
No one in the paintaing is looking at anyone else; just looking into their own world. From a far seems like everyone is having a great time but how could they when no one is looking at each other? 

Lesson: As the painter of your plot, you should be careful about every single pattern and element of the plot so that you're a more effective communicator. 
...
- When have a y of 0, then both x and z values also have 0. Only two rows have values across x, y, z. 
- For the ones where there are values across, then x and z are very close. y is completely out there. 
...
Just record the y value as NA and then leave the rest of the data intact. 
...
- Imagine 100 columns and in every column 1% look at garbage data. If you do drop the entire row then you're going to remove tons of data. 
...
Corvette appears like 5 times so maybe this model has the best mileage. 
